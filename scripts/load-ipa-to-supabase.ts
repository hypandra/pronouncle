...
CREATE INDEX IF NOT EXISTS idx_ipa_pronunciations_word ON ipa_pronunciations(word);
CREATE INDEX IF NOT EXISTS idx_ipa_pronunciations_lang ON ipa_pronunciations(lang);`
...
async function checkTable() {
  const { data, error } = await supabase.rpc('exec_sql', { query: "SELECT column_name FROM information_schema.columns WHERE table_name = 'ipa_pronunciations' ORDER BY ordinal_position;" })
...
async function main() {
  ...
  const { data: tableData, error: tableError } = await supabase.from('ipa_pronunciations').select('*').limit(1)
  ...
  const { error } = await supabase.from('ipa_pronunciations').upsert([testRecord], { onConflict: 'word' })
  ...
  const { error: bulkError } = await supabase.from('ipa_pronunciations').upsert(filteredData, { onConflict: 'word' })
